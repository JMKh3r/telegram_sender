...
{                                                                              }
type
  TTLSCertificateList = array of AnsiString;

procedure TLSCertificateListAppend(var List: TTLSCertificateList; const A: AnsiString);
...
procedure InitTLSPreMasterSecret(
          var PreMasterSecret: TTLSPreMasterSecret;
          const ClientVersion: TTLSProtocolVersion);

function  TLSPreMasterSecretToStr(const PreMasterSecret: TTLSPreMasterSecret): AnsiString;

procedure InitTLSEncryptedPreMasterSecret(
          var EncryptedPreMasterSecret: AnsiString;
...
type
  TTLSClientKeyExchange = record
    EncryptedPreMasterSecret  : AnsiString;
...
function  EncodeTLSHandshakeFinished(var Buffer; const Size: Integer;
          const MasterSecret: AnsiString;
          const ProtocolVersion: TTLSProtocolVersion;
          const HandshakeData: AnsiString;
...
function EncodeTLSOpaque16(var Buffer; const Size: Integer; const A: AnsiString): Integer;
var P : PByte;
    N, L : Integer;
begin
  N := Size;
...
function EncodeTLSOpaque24(var Buffer; const Size: Integer; const A: AnsiString): Integer;
var P : PByte;
    N, L : Integer;
begin
  N := Size;
...
function DecodeTLSOpaque16(const Buffer; const Size: Integer; var A: AnsiString): Integer;
var P : PByte;
    N, L : Integer;
begin
  N := Size;
...
function DecodeTLSOpaque24(const Buffer; const Size: Integer; var A: AnsiString): Integer;
var P : PByte;
    N, L : Integer;
begin
  N := Size;
...
{ by two length bytes.                                                         }
{                                                                              }
procedure InitTLSPreMasterSecret(var PreMasterSecret: TTLSPreMasterSecret;
          const ClientVersion: TTLSProtocolVersion);
begin
  PreMasterSecret.client_version := ClientVersion;
  SecureRandomBuf(PreMasterSecret.random, 46);
end;
...
var B : array[0..1023] of Byte;
    L : Integer;
begin
  L := RSAEncrypt(rsaetPKCS1, RSAPublicKey, PreMasterSecret, SizeOf(PreMasterSecret), B, SizeOf(B));
...
begin
  if SenderIsClient then
    ...
    T := #$53#$52#$56#$52;
  M := MD5DigestToStrA(
           CalcMD5(MasterSecret + DupCharA(#$5C, 48) +
               MD5DigestToStrA(
                   CalcMD5(HandshakeData + T + MasterSecret + DupCharA(#$36, 48)))));
  S := SHA1DigestToStrA(
           CalcSHA1(MasterSecret + DupCharA(#$5C, 40) +
               SHA1DigestToStrA(
                   CalcSHA1(HandshakeData + T + MasterSecret + DupCharA(#$36, 40)))));
...
function CalcTLSVerifyData(
         const ProtocolVersion: TTLSProtocolVersion;
         const MasterSecret: AnsiString;
         const HandshakeData: AnsiString): AnsiString;
var VDL : Integer;
    D1 : T128BitDigest;
    D2 : T160BitDigest;
    D3 : T256BitDigest;
